...
router.post('/register',upload.none(), function(request, response) {
  ...
  error = '';
  if (name === '') error += 'Поле Имя обязательно для заполнения. ';

  if (email === '') error += 'Поле E-Mail адрес для заполнения. ';

  if(password === '') error += 'Поле Пароль обязательно для заполнения.';

  //если ошибка сформирована...
  if(error !== '') response.json({success: false, error});//отправляем ошибку
